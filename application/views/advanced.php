...
.exgold {
  ...
  transition: transform 1s ease;  /* ← fixed */
  margin-bottom:6px;
  padding-bottom:2px;
  transition:all ease .5s ease;
}
.exgold:hover{
...
.exgoldd{
  ...
  margin-left: 2px;font-size: 21px; padding: 4px 15px; background: linear-gradient(45deg, #FFD700, #FFA500); color: black; border-radius: 20px; font-weight: bold; box-shadow: 0 0 5px rgba(255,215,0,0.8);
}
...
@media (max-width: 768px) {
  ...
  .logout-btn { padding: 6px 14px; font-size: 14px; }
}
...
<body>
  ...
  <div class="navbar-logo" >
    Xfinity
    <?php if (strcasecmp($this->session->userdata('membership'), 'gold membership') != 0): ?>

      <a href="<?php echo site_url('Home/profile'); ?>" class="exgold">
        Explore GOLD
      </a>
      
    <?php else: ?>
      <span class="exgoldd">
        <i class="fa-solid fa-crown" style="margin-right: 2px;"></i> GOLD
      </span>
    <?php endif; ?>
  </div>
  ...
  <?php if ($isLoggedIn): ?>
    <div class="user-info" style="cursor:pointer;" onclick="toggleSidebar()">
      <i class="fa-solid fa-user-astronaut" style="color:white; font-size:30px;"></i>
      <span class="username" style="font-size:19px;"><?php echo html_escape($userName); ?></span>
    </div>
  <?php else: ?>
    <div class="user-info">
      <a href="<?php echo site_url('login'); ?>" class="logout-btn" style="background: #007bff;">Login</a>
    </div>
  <?php endif; ?>
</nav>
...
<div class="sidebar" id="sidebar">
  ...
  <div class="profile-section">
    ...
    <p style="margin-top: 10px; font-weight: 500;"><?php echo html_escape($membership); ?></p>
  </div>
